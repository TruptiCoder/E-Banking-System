  ...
  useEffect(() => {
    async function loadAll() {
      try {
        ...
        const [accs, fdData] = await Promise.all([
          getAccountsAPI(auth.customerId),
          getFDsAPI(auth.customerId),
        ]);
        setAccounts(accs);
        setFDs(fdData);
        if (accs.length > 0)
          setTransactions(await getRecentTransactionsAPI(accs[0].accountId));
      } catch (err) {
    ...
    <div className="warm-page">
      ...
      <div className="warm-banner mb-4">
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <p className="warm-banner-date">
            {activeAccts} active account{activeAccts !== 1 ? "s" : ""}
          </p>
        </div>
      </div>

      {error && (
        <div className="warm-alert warm-alert-danger mb-4">
      ...
      <div className="mb-4">
        ...
        <div className="row g-3">
          {QUICK_ACTIONS.map((a) => (
            <div
              key={a.path}
              className="col-6 col-sm-4 col-lg-3 col-xl-auto"
              style={{ minWidth: 115 }}
            >
              <button
              ...
              >
                ...
                <span style={{ fontSize: 11.5, color: "var(--cafe-noir)" }}>
                  {a.label}
                </span>
              </button>
            </div>
          ...
          <div className="warm-card p-4">
            <div className="row g-0 text-center">
              {[
                ...
                {
                  label: "Avg. Rate",
                  value: `${(activeFDs.reduce((s, f) => s + f.interestRate, 0) / activeFDs.length).toFixed(2)}% p.a.`,
                  color: "#2d7a56",
                },
              ].map((item, i, arr) => (
                <div
